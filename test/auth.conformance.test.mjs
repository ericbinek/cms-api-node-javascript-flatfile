...
import { startServer, login, buildPayload, postEntity, setAuthToken } from './_helpers.mjs';
...
function req(bearer, method, path, body) {
  const headers = {};
...
async function createAs(bearer, entity, base, overrides = {}) {
...
test('createdBy is set to the creator and an author may modify only own records', async () => {
  const mine = await (await createAs(token.author, "BlogPosting", "/blog-postings")).json();
  const theirs = await (await createAs(token.author2, "BlogPosting", "/blog-postings")).json();
...
test('createdBy never appears in any entity response', async () => {
  const created = await (await createAs(token.admin, "BlogPosting", "/blog-postings")).json();
  assert.equal('createdBy' in created, false);
  const got = await (await req(token.admin, 'GET', `${"/blog-postings"}/${created.id}`)).json();
  assert.equal('createdBy' in got, false);
  const list = await (await req(token.admin, 'GET', `${"/blog-postings"}?limit=100`)).json();
...
test('system and internal fields are rejected in a write body with 400', async () => {
  ...
  for (const field of ['id', 'dateCreated', 'dateModified', 'createdBy']) {
    const payload = { ...(await buildPayload(server.baseUrl, "BlogPosting")), [field]: field === 'id' ? '00000000-0000-0000-0000-000000000000' : 'x' };
...
test('a freshly created BlogPosting has the initial status', async () => {
  const created = await (await createAs(token.author, "BlogPosting", "/blog-postings")).json();
...
test('author may run the initial transition but not the editor-only one', async () => {
  const item = await (await createAs(token.author, "BlogPosting", "/blog-postings")).json();
  // author: initial -> authorTo allowed
  const a = await req(token.author, 'PUT', `${"/blog-postings"}/${item.id}`, { ["creativeWorkStatus"]: "Pending" });
  assert.equal(a.status, 200);
  assert.equal((await a.json())["creativeWorkStatus"], "Pending");
...
test('an unmodelled transition is forbidden', async () => {
  const item = await (await createAs(token.editor, "BlogPosting", "/blog-postings")).json();
...
test('anonymous reads see only public records; non-public detail is 404', async () => {
  const item = await (await createAs(token.admin, "BlogPosting", "/blog-postings")).json();

  // Not yet public: hidden from anonymous list and detail (404, not 403).
  const hiddenList = await (await req(null, 'GET', `${"/blog-postings"}?limit=100`)).json();
  assert.equal(hiddenList.items.some((i) => i.id === item.id), false);
  ...
  const publish = { ["creativeWorkStatus"]: "Published" };
  ...
  const shownList = await (await req(null, 'GET', `${"/blog-postings"}?limit=100`)).json();
  assert.equal(shownList.items.some((i) => i.id === item.id), true);
  const detail = await req(null, 'GET', `${"/blog-postings"}/${item.id}`);
  assert.equal(detail.status, 200);
  assert.equal('createdBy' in (await detail.json()), false);
});

test('an entity without a status enum is anonymously readable and unrestricted by workflow', async () => {
  const created = await (await createAs(token.admin, "Person", "/persons")).json();
