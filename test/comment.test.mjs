...
test(`${ENTITY}: sort by numeric field "upvoteCount" orders numerically with missing last`, async () => {
  ...
  const presentVals = mine
    .filter((i) => typeof i["upvoteCount"] === 'number')
    .map((i) => i["upvoteCount"]);
...
test(`${ENTITY}: leading/trailing whitespace is trimmed on create`, async () => {
  ...
  const r = await postEntity(server.baseUrl, ENTITY, payload);
  assert.equal(r.status, 201, `expected 201, got ${r.status}`);
  assert.equal((await r.json())["text"], 'trimmed value');
});
...
test(`${ENTITY}: control characters are stripped on create`, async () => {
  ...
  const r = await postEntity(server.baseUrl, ENTITY, payload);
  assert.equal(r.status, 201, `expected 201, got ${r.status}`);
  assert.equal((await r.json())["text"], 'cleaned');
});
...
test(`${ENTITY}: multiline field "text" keeps internal newlines`, async () => {
  ...
  const r = await postEntity(server.baseUrl, ENTITY, payload);
  assert.equal(r.status, 201, `expected 201, got ${r.status}`);
  assert.equal((await r.json())["text"], 'first line\nsecond line');
});
