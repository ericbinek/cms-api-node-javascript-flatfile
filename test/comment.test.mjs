...
import { startServer, buildPayload, postEntity } from './_helpers.mjs';
...
test(`${ENTITY}: GET by id embeds "author" as an object; list stays flat`, async () => {
  ...
  const got = await (await fetch(`${server.baseUrl}${BASE}/${created.id}`)).json();
  ...
  const list = await (await fetch(`${server.baseUrl}${BASE}?limit=100`)).json();
